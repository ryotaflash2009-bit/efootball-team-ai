...
// 通常運用(secure-connect.ps1経由)ではいずれも設定されず、既定値(実DBパス・実件数)の
// ままなので、本番の挙動・安全性(件数一致ゲート等)は一切変わらない。
...
const EXPECTED_ANALYSIS_COUNT = Number(process.env.MIGRATION_EXPECTED_ANALYSIS_COUNT || 19);
