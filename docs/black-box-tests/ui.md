...
| PASS | 空状態: スカッドの空はピッチプレビュー＋作成CTAの文言が辞書に存在する |  |
...
| PASS | ホーム: 実データ指標（World カード / 監督 = APIの件数） | managers=67 world=13009 |
| PASS | ホーム: World データの取り込み日時(World と監督を区別した表示) |  |
...
| PASS | 回帰: 監督総数 / World 総数の表示がAPIの件数と一致 | managers=67 world=13009 |
